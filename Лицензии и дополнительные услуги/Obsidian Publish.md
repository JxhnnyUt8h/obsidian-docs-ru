...
For instructions on how to publish your notes from within the Obsidian app, refer to the [[Публикация|Publish plugin]].

### Privacy

You have total control over what to publish via the [[Публикация|Publish plugin]] - either your entire vault or just selected notes within your vault. As for any notes you choose **not** to publish, you can be confident that those notes remain completely private, and Obsidian Publish has no knowledge of them.
...
There are several components which can be turned on or off from the [[Публикация|Publish plugin]]. Changes to these options will be reflected on your published site once the cache clears, typically within 5 minutes.
...
Displays a local graph of the page currently open. This uses the same rendering engine as the [[Граф]] plugin in Obsidian.

##### Table of contents

Displays a table of contents for the page currently open. The list is generated from headings within the page, similar to the [[Структура]] plugin. Viewers navigating the note will see the corresponding heading highlighted as they scroll, and can click on another heading to immediately navigate to it.
...
When hovering over links, their content is displayed within a popup box. This works similarly to the [[Предпросмотр страницы]] plugin in Obsidian.
